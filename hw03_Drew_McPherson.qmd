...
```{r}
# Clean column names
state_table = raw_table %>% clean_names()


```

1.  Make sure the class of each column is correct\
2.  Replace `__` with `NA`
...
# Extract the first table (State and territory rankings)
raw_table <- tables[[1]] %>% html_table(fill = TRUE)

# Clean column names
state_table <- raw_table %>% clean_names()

# Print cleaned column names so we know exactly what they are
print(names(state_table))
